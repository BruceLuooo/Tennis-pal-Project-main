...
function Courts() {
  ...
  useEffect(() => {
    const courtData = async() => {
      try{
        setCurrentPage(1)
        const {data} = await axios.get(`http://localhost:5000/api/allCourts?search=${search}`)
  ...
  useEffect(() => {
    const courtData = async() => {
      try{
        const {data} = await axios.get(`http://localhost:5000/api/allCourts?&page=${currentPage}&search=${search}`)
        setCourts(data.courts)
      } catch(error){
        console.log(error)
      }
