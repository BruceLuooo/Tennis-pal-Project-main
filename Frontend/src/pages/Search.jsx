...
function Search() {
  ...
  useEffect(() => {
    const getAllUsers = async() => {
      try{
        ...
        filter.locations.forEach(location => params.append('locations[]', location));
        // http://localhost:5000/api/allUsers?level[]=1&level[]=2
        const {data} = await  axios.get(`http://localhost:5000/api/allUsers?page=${currentPage}`, { params })
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/api/allCourts/list`).then(({data}) => setLocationsList(data))
  }, [])
        ...
        <div className='filter-container'>
          ...
          {playerLevel.map((level) => {
            return (
              <div className='filter'>
                <input type="checkbox" id='level' value={level} onClick={updateLevelFilter}/>
                <p>Level {level}</p>
              </div>
            )
          })}
          ...
          <div className="different-filters">
            {locationsLists.map((location) => {
              return (
                <div className='filter'>
                  <input type="checkbox" id='locations' value={location.name} onClick={updateLocationFilter}/>
                  <p>{location.name}</p>
                </div>
              )
            })}
      ...
      <div className="searchPlayer-container">
        <div className="profiles-container">
          {users.filter((current) => current._id !== user._id  ).map((current) => (
            <ProfileCards key={current.name} searchedUser={current} currentUser ={user}/>
          ))}
        </div>
        
        <div className={numOfPages === 0 ? 'blank' : 'page-btn-container'}>
          <button className='pageBtn' onClick={prevPage}>Prev</button> 
          {totalPages.map((data) => {
            return (
            <button 
              type='button' 
