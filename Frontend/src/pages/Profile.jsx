...
function Profile() {
	...
	const { name, email, level, password, locations, avatar } = userData;
	...
	useEffect(() => {
		...
		axios
			.get('http://localhost:5000/api/allUsers/totalUsers')
	...
	const deleteCourt = async courtName => {
		try {
			const { data } = await axios.post(
				'http://localhost:5000/api/allCourts/deleteCourt',
				{ name: courtName },
			);
			setRefresh(data);
	...
	const deleteUser = async user => {
		try {
			const { data } = await axios.post(
				'http://localhost:5000/api/allUsers/deleteUser',
				{ _id: user },
			);
			setRefresh(data);
	...
	const AdimRole = async user => {
		try {
			if (user.isAdmin === false) {
				const { data } = await axios.post(
					'http://localhost:5000/api/allUsers/makeAdmin',
					{ _id: user._id },
				);
				setRefresh(data);
				...
				const { data } = await axios.post(
					'http://localhost:5000/api/allUsers/deleteAdmin',
					{ _id: user._id },
				);
				setRefresh(data);
